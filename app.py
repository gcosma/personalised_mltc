...
# First, add these new functions after the existing analysis functions but before main():

def filter_trajectories(data, gender, age_group, min_or=2.0, min_prevalence=0.0):
    """Filter trajectories based on specified criteria"""
    filtered_data = data[
        (data['OddsRatio'] >= min_or) &
        ((data['PairFrequency'] / data['TotalPatientsInGroup'].iloc[0]) * 100 >= min_prevalence)
    ].copy()
    
    # Calculate additional metrics
    total_patients = filtered_data['TotalPatientsInGroup'].iloc[0]
    or_range = [filtered_data['OddsRatio'].min(), filtered_data['OddsRatio'].max()]
    min_prev_count = filtered_data['PairFrequency'].min()
    min_prev_percent = (min_prev_count / total_patients) * 100
    pair_count = len(filtered_data)
    total_or_sum = filtered_data['OddsRatio'].sum()
    ...
    summary_stats = {
        ...
        'OR Range': or_range,
        'Min Prevalence': f"{min_prev_percent:.2f}% ({min_prev_count} patients)",
        'Condition Pairs': pair_count,
        'Total OR Sum': f"{total_or_sum:.2f}"
    }
    
    return filtered_data, summary_stats

def create_filter_visualization(filtered_data):
    """Create visualization for filtered trajectories"""
    # Create a figure with two subplots
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
    
    # Plot 1: Odds Ratio Distribution
    sns.histplot(data=filtered_data, x='OddsRatio', bins=20, ax=ax1)
    ax1.set_title('Distribution of Odds Ratios')
    ax1.set_xlabel('Odds Ratio')
    ax1.set_ylabel('Count')
    
    # Plot 2: Prevalence vs Odds Ratio
    total_patients = filtered_data['TotalPatientsInGroup'].iloc[0]
    prevalence = (filtered_data['PairFrequency'] / total_patients) * 100
    
    scatter = ax2.scatter(filtered_data['OddsRatio'], prevalence,
                         c=filtered_data['PairFrequency'], cmap='viridis',
                         alpha=0.6)
    ax2.set_title('Prevalence vs Odds Ratio')
    ax2.set_xlabel('Odds Ratio')
    ax2.set_ylabel('Prevalence (%)')
    
    # Add colorbar
    plt.colorbar(scatter, ax=ax2, label='Pair Frequency')
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if data is not None:
            ...
            with tabs[4]:
                st.header("Filtered Trajectory Analysis")
                st.markdown("""
                Analyze disease trajectories based on specific demographic and statistical criteria.
                """)
                ...
                with filter_col1:
                    ...
                    min_or = st.slider(
                        "Minimum Odds Ratio",
                        2.0, 10.0, 2.39,
                        help="Minimum odds ratio threshold"
                    )
                    ...
                    filter_button = st.button(
                        "🎯 Apply Filters",
                        key="apply_filters",
                        help="Apply selected filters to the data"
                    )

                with filter_col2:
                    if filter_button:
                        with st.spinner("Applying filters..."):
                            filtered_data, summary_stats = filter_trajectories(
                                data,
                                gender,
                                age_group,
                                min_or,
                            ...
                            summary_md = f"""
                            - **Gender:** {summary_stats['Gender']}
                            - **Age group:** {summary_stats['Age Group']}
                            - **Total patients:** {summary_stats['Total Patients']:,}
                            - **Odds ratio range:** [{summary_stats['OR Range'][0]:.2f} - {summary_stats['OR Range'][1]:.2f}]
                            - **Observed minimum prevalence:** {summary_stats['Min Prevalence']}
                            - **Number of condition pairs:** {summary_stats['Condition Pairs']}
                            - **Total sum of odds ratios:** {summary_stats['Total OR Sum']}
                            """
                            st.markdown(summary_md)
                            
                            # Display filtered data
                            st.markdown("### Filtered Trajectories")
                            st.dataframe(
                            ...
                            fig = create_filter_visualization(filtered_data)
                            st.pyplot(fig)
